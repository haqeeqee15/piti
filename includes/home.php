    ...
    <table class="table">
      ...
      <tfoot>
        <tr>
          <th></th>
          <th>Total</th>
          <th>Rp <span id="total-expense"></span></th>
        </tr>
      </tfoot>
    </table>
  </div>
    ...
    const rows = data.map((item, i) => {
      ...
      return `
        <tr>
          ...
          <td>${item.category}</td>
          <td>Rp ${item.total_expense.toLocaleString()}</td>
        </tr>
      `;
    }).join('');
    document.getElementById('expense-table-body').innerHTML = rows;
    document.getElementById('total-expense').innerHTML = total.toLocaleString();;
  });
</script>
...
addButton.addEventListener('click', () => {
  ...
  setTimeout(() => {
    window.location.href = "add-Pengeluaran.php";
  }, 200);
});
</script>
